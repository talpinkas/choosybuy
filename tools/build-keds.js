// ============================================================
// CHOOSY — Keds (keds.co.il) catalog builder — AUTOMATED (Shopify).
// ============================================================
// Pure kids retailer on Shopify. Like Fox: fetch the 4 gender×age collections
// (בייבי-* = 0-2, בנים/בנות = 2-8); category from product_type + title.
//   node tools/build-keds.js
// ============================================================
'use strict';
var fs = require('fs');
...
var SITE = 'https://keds.co.il';
var BRAND = 'Keds';

var COLLECTIONS = [
  { handle: 'בייבי-בנים', gender: 'boy', age: '0-2' },
  { handle: 'בייבי-בנות', gender: 'girl', age: '0-2' },
  { handle: 'בנים', gender: 'boy', age: '2-8' },
  { handle: 'בנות', gender: 'girl', age: '2-8' }
];
...
// product_type + title -> category, or null to skip (shoes/socks/underwear/accessories)
function categoryOf(s) {
  if (/גרב|סניקרס|נעל|תחתונ|בוקסר|כובע|אביזר|גרבי/.test(s)) return null; // accessories/footwear
  ...
  if (/חולצ|גופי|פוטר|סווטשירט|סווט שירט|מקטורן|סריג|\bטופ\b|טישרט|טי-?שירט|חליפ|חליפת גן|מיינט/.test(s)) return 'tops';
  return null;
}
...
async function main() {
  ...
  for (var col of COLLECTIONS) {
    ...
    prods.forEach(function (p) {
      ...
      if (!cur || !img) return;
      var id = 'keds-' + col.gender + '-' + col.age + '-' + category;
      (buckets[id] = buckets[id] || []).push({
        id: 'keds-' + (p.id || p.handle),
  ...
  Object.keys(buckets).sort().forEach(function (id) {
    ...
    var m = id.match(/^keds-(boy|girl)-(0-2|2-8)-(\w+)$/);
    ...
    written.push(id.replace('keds-', '') + ' (' + products.length + ')');
  });
